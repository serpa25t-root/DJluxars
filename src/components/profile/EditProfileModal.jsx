...
const AvatarFrame = ({ src, alt, size = 'h-24 w-24', icon = 'h-6 w-6' }) =>
  src ? (
    <img src={src} alt={alt} className={`${size} rounded-full object-cover ring-2 ring-white/10`} />
  ) : (
    <span className={`${size} rounded-full bg-zinc-800 border-2 border-white/10 flex items-center justify-center text-xl font-bold text-white`}>
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-xl max-h-[90vh] overflow-hidden rounded-2xl border border-zinc-800 bg-zinc-950 shadow-2xl shadow-black/60 flex flex-col animate-[scaleIn_250ms_var(--ease-out-expo)_both]">
        <div className="h-[2px] w-full bg-gradient-to-r from-zinc-600 via-zinc-500 to-zinc-600" />
