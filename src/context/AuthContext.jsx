...
export const AuthProvider = ({ children }) => {
  ...
  const register = useCallback(async (userData) => {
    ...
    try {
      // Ruta verificada en users/urls.py -> api/users/register/ con slash final
      const res = await api.post('users/register/', payload)
      return res.data
    } catch (err) {
      const msg = spanishErrorMap(extractErrorMsg(err, 'No se pudo crear la cuenta. Verifica los datos.'))
      if (msg.toLowerCase().includes('email') && msg.toLowerCase().includes('exist')) throw new Error('El correo ya está registrado.')
      throw new Error(msg)
    }
  }, [])
