...
const SettingRow = ({ label, description, children }) => (
  <div className="flex items-center justify-between gap-4 rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-4 backdrop-blur-xl hover:border-red-500/50 transition-all duration-300">
    ...
    <div className="mx-auto max-w-3xl animate-[fadeIn_300ms_ease-out]">
      ...
      <div className="flex items-center gap-6 mb-8 border-b border-zinc-900">
        {tabs.map((t) => (
          <button
            ...
            className={`text-sm font-medium border-b-2 transition-colors pb-1 ${activeTab === t.id ? 'border-red-600 text-white' : 'border-transparent text-zinc-400 hover:text-zinc-300'}`}
          >
            {t.label}
          </button>
        ))}
      ...
      {activeTab === 'general' && (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <SettingRow label="Zona horaria" description="Tu ubicación actual">
              <span className="text-sm text-zinc-400">America/Bogota (COT)</span>
            </SettingRow>
            <SettingRow label="Moneda" description="Para mostrar precios">
          ...
          <div className="space-y-3">
            <SettingRow label="Tema" description="Apariencia de la interfaz">
              <select
                value={theme}
            ...
            <SettingRow label="Calidad de imagen" description="Menor = carga más rápida">
              <select
                defaultValue="high"
                className="rounded-xl border border-zinc-800 bg-zinc-900/60 px-3 py-2 text-sm text-white focus:border-red-600/50 focus:outline-none focus:ring-2 focus:ring-red-600/20 transition-colors min-w-[120px]"
              >
          ...
          <div className="space-y-3">
            <SettingRow label="Correo electrónico" description={user?.email || 'No configurado'}>
            ...
            <SettingRow label="Contraseña" description="Último cambio hace 30 días">
              <button className="rounded-full border border-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors">
                Cambiar
              </button>
            </SettingRow>
            <SettingRow label="Eliminar cuenta" description="Esta acción es irreversible">
      ...
      {activeTab === 'notifications' && (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            ...
            <SettingRow label="Notificaciones por email" description="Resumen semanal de actividad">
          ...
          <div className="space-y-3">
            ...
            <SettingRow label="Marketing" description="Promociones y ofertas especiales">
      ...
      {activeTab === 'privacy' && (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <SettingRow label="Perfil público" description="Visible en el catálogo de fotógrafos">
              <Toggle checked={profileVisible} onChange={(v) => handleNotifToggle(setProfileVisible, 'luxarts_profile_visible', v)} />
            </SettingRow>
            <SettingRow label="Mostrar estado en línea" description="Otros usuarios pueden ver si estás activo">
              <Toggle checked={showOnline} onChange={(v) => handleNotifToggle(setShowOnline, 'luxarts_show_online', v)} />
            </SettingRow>
            <SettingRow label="Mostrar teléfono" description="Visible en tu perfil público">
          ...
          <div className="space-y-3">
            <SettingRow label="Quién puede enviarte mensajes" description="Controla quién puede contactarte">
          ...
          <div className="space-y-3">
            <SettingRow label="Descargar mis datos" description="Solicita una copia de tu información">
              <button className="rounded-full border border-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors">
                Solicitar
              </button>
            </SettingRow>
            <SettingRow label="Sesiones activas" description="2 dispositivos conectados">
              <button className="rounded-full border border-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors">
                Gestionar
              </button>
            </SettingRow>
      ...
      {activeTab === 'help' && (
        <div className="space-y-6">
          ...
          <div className="space-y-3">
            <SettingRow label="Centro de ayuda" description="Preguntas frecuentes y guías">
              <button className="rounded-full border border-zinc-800 px-3 py-1.5 text-xs font-medium text-zinc-400 hover:text-white hover:border-zinc-700 transition-colors">
                Abrir
              </button>
            </SettingRow>
            <SettingRow label="Contactar soporte" description="Chat en vivo o email">
          ...
          <div className="rounded-2xl border border-zinc-800/80 bg-zinc-900/50 p-6 text-center backdrop-blur-xl">
